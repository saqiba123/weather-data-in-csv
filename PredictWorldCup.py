...
# load data sets
world_cup = pd.read_csv('World Cup 2019 Dataa.csv')
results = pd.read_csv('results.csv')

print(results.head())

# Filter the matches played by pakistan
...
# delete columns that wll not affect match outcomes
df_teams_2010 = df_teams.drop(['date', 'Margin', 'Ground'], axis=1)
print(df_teams_2010.head())
"""feature Engineering ends here

Now let build the model"""

# Building the model
# the prediction label: The winning_team column will show label "1" Team 1 has won and label "2" if the away team has won.
...
semi = [('New Zealand', 'India'), ('England', 'Australia')]
print(semi)
...
print(clean_and_predict(semi, ranking, final, logreg))
# Finals
finals = [('India', 'England')]
print(finals)
print(clean_and_predict(finals, ranking, final, logreg))
